...
class SwinIRLoader:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "model_type": (["classicalSR", "lightweightSR", "realSR", "denoising"], ),
                "upscale": ("INT", {"default": 4, "min": 1, "max": 8}),
                "window_size": ("INT", {"default": 8, "min": 4, "max": 32}),
                "embed_dim": ("INT", {"default": 180, "min": 48, "max": 512, "step": 12}),
                "depths": ("STRING", {"default": "6, 6, 6, 6, 6, 6"}),
                "num_heads": ("STRING", {"default": "6, 6, 6, 6, 6, 6"}),
                "mlp_ratio": ("FLOAT", {"default": 2.0, "min": 1.0, "max": 8.0, "step": 0.5}),
            }
        }
    ...
    def load_model(self, model_name, model_type, upscale, window_size, embed_dim, depths, num_heads, mlp_ratio):
        ...
        if model_type == 'classicalSR':
            ...
            if upscale != 1:
                print(f"Warning: model_type is denoising but upscale is {upscale}. SwinIR denoising usually implies upscale=1.")
        ...
        for image in images:
            ...
            with torch.no_grad():
                ...
                if h <= tile_size and w <= tile_size:
                    ...
                    output = self.tile_process(img_tensor, swinir_model, tile_size, overlap)
